...
select c.id,c.column_name,t.id,t.table_name,-1,1,table_wh_type from assets_meta_column c
...
where t.id = {$last_insert_id}";
  ...
  select id,column_id,column_id from assets_table_column_relation where table_id = {$last_insert_id}";
  echo $sql.PHP_EOL;
  $db->query($sql);
  // die;
