...
$mgs = '';
if($sum_process >= 8000){
    $mgs .= "ERP报警 check_process 生产库总进程数{$sum_process}。";
...
if(count($not_sleep_process) > 20){
    ...
    $mgs .= "ERP报警 check_process 执行中的sql超过有{$count_not_sleep_process}个。";
...
if(count($slow1_process) > 5){
    ...
    $mgs .= "ERP报警 check_process 执行中的sql超过有{$count_slow1_process}个执行10秒。";
...
if(count($slow2_process) > 20){
    ...
    $mgs .= "ERP报警 check_process 执行中的sql有{$count_slow2_process}个并且执行20秒。";
