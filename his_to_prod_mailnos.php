...
for ($i = $begin; $i < $end; $i++) {
    ...
    move_mailnos($facility_id);
    move_shipment_package($facility_id);
    move_shipment($facility_id);
}
echo date("Y-m-d H:i:s", time())."  ".$start_time . " to" . $end_time . " move finished" . PHP_EOL;


function move_mailnos($facility_id){
    global $erp_ddyun_db, $erp_ddyun_history_db, $erp_ddyun_history_db_conf, $erp_ddyun_db_conf, $start_time, $end_time;
    $limit = 0;
    while (true) {
        $select_sql = "select m.* from mailnos m  where m.created_time >= '{$start_time}' and m.created_time < '{$end_time}' and m.facility_id = {$facility_id} order by m.id limit {$limit}, 200";
        echo $erp_ddyun_db_conf['name'] . "   " . $select_sql . PHP_EOL;
        $limit += 200;
        $mailnos_list = $erp_ddyun_history_db->getAll($select_sql);
...
function move_shipment_package($facility_id){
    global $erp_ddyun_db, $erp_ddyun_history_db, $erp_ddyun_history_db_conf, $erp_ddyun_db_conf, $start_time, $end_time;
    $limit = 0;
    while(true){
        $select_sql = "select * from shipment_package where created_time >= '{$start_time}' and created_time < '{$end_time}' and facility_id = {$facility_id} order by id limit {$limit}, 200";
        echo date("Y-m-d H:i:s", time())."  ".$erp_ddyun_db_conf['name']."   ".$select_sql.PHP_EOL;
        $limit += 200;
        $shipment_package_list = $erp_ddyun_history_db->getAll($select_sql);
...
function move_shipment($facility_id){
    global $erp_ddyun_db, $erp_ddyun_history_db, $erp_ddyun_history_db_conf, $erp_ddyun_db_conf, $start_time, $end_time;
    $limit = 0;
    while(true){
...
where mgs.shipping_time < '{$start_time}' and mgs.shipping_time >= '{$end_time}' and mgs.facility_id = {$facility_id} order by shipment_id limit {$limit}, 200";
        $limit += 200;
        echo $id_sql.PHP_EOL;
        $ids = $erp_ddyun_history_db->getAll($id_sql);
        if(empty($ids)){
            break;
        }
        ...
        $shipment_list = $erp_ddyun_history_db->getAll($select_shipment_sql);
        ...
        if(!empty($shipment_list)){
            $erp_ddyun_db->query($insert_shipment_sql);
            echo  date("Y-m-d H:i:s")."  ".$erp_ddyun_db_conf['name']."  insert ".count($shipment_list) ." shipment into prod".PHP_EOL;
        }


        $select_shipment_extension_sql = "select * from multi_goods_shipment_extension where shipment_id in ({$shipment_ids})";
        $shipment_extension_list = $erp_ddyun_history_db->getAll($select_shipment_extension_sql);
        ...
        if(!empty($shipment_extension_list)){
            $erp_ddyun_db->query($insert_shipment_extension_sql);
            echo  date("Y-m-d H:i:s")."  ".$erp_ddyun_db_conf['name']."  insert ".count($shipment_extension_list) ." shipment_extension into prod".PHP_EOL;
        }

        $select_mgsg_sql = "select * from multi_goods_shipment_goods where order_goods_id in ({$order_goods_ids})";
        $mgsg_list = $erp_ddyun_history_db->getAll($select_mgsg_sql);
        ...
        $insert_mgsg_sql .= " ON DUPLICATE KEY UPDATE
            ...
            ";
        if(!empty($mgsg_list)){
            $erp_ddyun_db->query($insert_mgsg_sql);
            echo  date("Y-m-d H:i:s")."  ".$erp_ddyun_db_conf['name']."  insert ".count($mgsg_list) ." multi_goods_shipment_goods into prod".PHP_EOL;
        }

        $select_finance_detail_sql = "select * from finance_detail where order_goods_id in ({$order_goods_ids})";
        $finance_detail_list = $erp_ddyun_history_db->getAll($select_finance_detail_sql);
        ...
        if(!empty($finance_detail_list)){
            $erp_ddyun_db->query($insert_finance_detail);
            echo  date("Y-m-d H:i:s")."  ".$erp_ddyun_db_conf['name']."  insert ".count($finance_detail_list) ." finance_detail into prod".PHP_EOL;
        }
    }
}
