...
$py_defines = $db->getAll("select *,count(1) c from crawler_monitor_task_define where id > 49 and crawler_type = 'PY' group by path_id having c=1");
$rpa_defines = $db->getAll("select *,count(1) c from crawler_monitor_task_define where id > 49 and crawler_type = 'RPA' group by path_id having c=1");
$paths = $db->getCol("select id from crawler_manage_node_info");
$alerts = $db->getCol("select id from crawler_elihu_alert_detail");
$records = $db->getAll("select * from crawler_monitor_task_execute_record");
...
foreach ($records as $record) {
  ...
  $result = $db->getRow("select * from crawler_monitor_task_execute_result where execute_id = {$record['record_id']}");
  var_dump($result);
  // foreach ($tables as $table) {
     if (empty($result)) {
    continue;
  }
    $type = $record['crawler_type'];
  var_dump($type);
  // die;
  ...
  select execute_id,crawler_type, app_id, account_id, 'FAIL', business_unit, task_id, mizar_group_id, shovel_record_id, alert_id, time_range, error_reason, start_time from crawler_monitor_task_execute_result where id = {$result['id']}";
  echo $sql.PHP_EOL;
  $db->query($sql);

    $sql = "replace into crawler_monitor_task_execute_result (execute_id,crawler_type, app_id, account_id, result_status, business_unit, task_id, mizar_group_id, shovel_record_id, alert_id, time_range, error_reason, start_time)
  values (".$record['record_id'].",'{$type}',{$count},".$accounts[random_int(0,count($accounts)-1)].",'FAIL','测试类目',{$count2},{$count2},{$count2},".$alerts[random_int(0,count($alerts)-1)].",'{$result['time_range']}','lw_test_".$count."','".date("Y-m-d H:i:s")."')";
  echo $sql.PHP_EOL;
  $db->query($sql);
  // die;
}
