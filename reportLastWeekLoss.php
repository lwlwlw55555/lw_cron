...
$this_monday = date('Y-m-d', strtotime('-1 monday', time()));
$sql = "
		select 
		    facility_id,
		    count(1) shop_count,
        ...
        having  max_expire_time > '{$last_monday}' and  max_expire_time < '{$this_monday}'
	";
...
foreach ($user_list as $key=>$item){
    ...
    $sql = "
		...
		from 
		     shop_data
		where 
			count_date >= date(date_sub('{$item['max_expire_time']}',interval 3 day))
			and count_date < date('{$item['max_expire_time']}')
            and facility_id = {$item['facility_id']}
	";
    $user = $db_user->getAll($sql)[0];
...
$key_array = array('user_name','shop_count','created_time','last_login_time','order_count','print_count','mobile');
$header = array(array('账号名','绑定店铺数','账号注册时间','最后登入时间','过期前3天订单量','过期前3天打单量','电话'));
send_erp_email('erp流失数据',$header,$key_array, $report_data,0,array("jpxiang@titansaas.com"));
