...
var_dump($shops);
    foreach ($shops as $shop) {
      ...
      $sql = "insert into sync_order_new_shop_job(shop_id,facility_id,status) values ({$shop},".$sync_db->getOne("select default_facility_id from shop where shop_id = {$shop}").",'DONE')";
      echo "select default_facility_id from shop where shop_id = {$shop}".PHP_EOL;
      $sync_db->query($sql);
      sleep(60);
    }
	}catch(Exception $e){
