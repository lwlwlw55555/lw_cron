...
while($is_begin || $count == 2000){
    $is_begin = false;
    $sql = "select o.platform_order_sn from order_exception_mapping m 
            inner join origin_order o on m.order_id = o.origin_order_id 
            left join mddomsuser.origin_transform_error_order ot on ot.platform_order_sn = o.platform_order_sn
            where order_exception_id = 10007 and enabled = 1 and ot.platform_order_sn is null";
    ...
    $start += $count;
    if ($count >0) {
        $sql = " insert into mddomsuser.origin_transform_error_order (shop_id,platform_order_sn,party_id,db,transform_type,transform_level)
        select shop_id,platform_order_sn,party_id,0,'add',1 from mddoms_0.origin_order where platform_order_sn in ('".implode("','", $ids)."')";
        echo $sql.PHP_EOL;
        $oms_db->query($sql);
    }
}
