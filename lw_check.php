...
$mod = isset($opt_params['mod'])?$opt_params['mod']:0;
...
$ss = $db->getAll("show tables");
    ...
    WHERE
     ...
     and se.enabled = 1
     and mod(s.shop_id,10) = {$mod}
     and s.platform_name = 'pinduoduo'
    order by s.`shop_id`
    ";
...
function check_sync_order($shop_id,$platform_shop_id,$secret,$platform_name,$access_token=null,$app_key=null,$facility_id){
        global $sync_db, $db;
        $result = getPlatformUnShippedOrderSnLw($shop_id,$platform_shop_id,$secret,$platform_name,$access_token,$app_key);
        // $result = getPlatformUnShippedOrderSnLw($shop_id);
...
function getPlatformUnShippedOrderSnLw($shop_id,$platform_shop_id,$secret,$platform_name,$access_token,$app_key){
    global $sync_db, $db;
    global $pdp_db;

    $pdp_db_conf = array(
        "host" => "10.1.13.152:3306",
        "user" => "pdp_sync",
        "pass" => "dasds1Sds",
        "charset" => "utf8",
        "pconnect" => "1",
    ...
    $pdp_db = ClsPdo::getInstance($pdp_db_conf);

     $pdp_db_1_conf = array(
        "host" => "10.1.15.27:3306",
        "user" => "pdp_sync",
        "pass" => "dasds1Sds",
        "charset" => "utf8",
        "pconnect" => "1",
    ...
    $pdp_db_1 = ClsPdo::getInstance($pdp_db_1_conf);

    $platform_shop_id = $sync_db->getOne("select platform_shop_id from shop where shop_id  = {$shop_id}");
    $pdp_rdis_name = $sync_db->getOne("select pdp_rds_name from shop_extension where shop_id = {$shop_id}");
    if (empty($pdp_rdis_name)) {
        return getPlatformUnShippedOrderSn($shop_id,$platform_shop_id,$secret,$platform_name,$access_token,$app_key);
    }
    if($pdp_rdis_name == 'pdp_sync_1'){
        $orders = $pdp_db->getCol("select order_sn from pdp_tb_trade where order_status = 1 and refund_status = 1 and mall_id = {$platform_shop_id}");
    }else if($pdp_rdis_name == 'pdp_sync_2'){
        $orders = $pdp_db_1->getCol("select order_sn from pdp_tb_trade where order_status = 1 and refund_status = 1 and mall_id = {$platform_shop_id}");
    }
    return $orders;
}
