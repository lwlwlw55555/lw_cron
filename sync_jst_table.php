...
function moveDataByPartyId($jst_db,$lw_db){
    ...
    $sql = "select distinct TABLE_NAME from information_schema.COLUMNS  where TABLE_SCHEMA = 'sys_info'";
    ...
    // moveOrderGoodsExtension($party_id, 'order_goods_extension', $jst_db, $jst_2_db, $jst_db_conf, $jst_2_db_conf, 'sys_info');
    // echo "[insert into db2, party_id: {$party_id}]" .date("Y-m-d H:i:s"). " sys_info party_id end" .PHP_EOL;
...
function moveData($from_db,$to_db,$table){
    ...
    $primay_key = null;
    if(empty($columns)){
        $columns_infos = execRetry($from_db, $from_conf, "show COLUMNS from sys_info.{$table}", 'getAll');
