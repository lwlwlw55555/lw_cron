...
$errors = $sync_db->getAll($sql_get_error_records);
foreach ($errors as $error) {
	...
	$result = null;
	if ('order' == $type) {
		$result = ExpressApiService::syncTaobaoPlatformOrderWithTimestamp($error['begin_time'],$error['end_time']);
		...
		while ($begin_time < $error['end_time']) {
			...
			$result = ExpressApiService::syncTaobaoPlatformGoodsWithTimestamp($begin_time,$end_time);
			if(!isset($result['code']) || $result['code'] > 0){
				echo "[]"."error: ";
		        var_export($result);
		        echo "[]".PHP_EOL;
