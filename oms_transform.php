...
while($is_begin || $count == 2000){
    $is_begin = false;
    $sql = "select id from
         origin_transform_error_order
         where transform_level = 1
      limit 2000";
     echo date("Y-m-d H:i:s")." sql".$sql;
    $list = $oms_user->getCol($sql);
    $count = count($list);
    $start += $count; 
    $sql = "replace into origin_transform_error_order_ignore (id, platform_order_sn, party_id, shop_id, db, transform_type, error_msg, transform_level, retry_count, created_time, last_updated_time, created_user_id, last_updated_user_id)
select id, platform_order_sn, party_id, shop_id, db, transform_type, error_msg, transform_level, retry_count, created_time, last_updated_time, created_user_id, last_updated_user_id from origin_transform_error_order where id in (".implode(",", $list).")";
    echo $sql.PHP_EOL;
    $oms_user->query($sql);
    $sql = "delete from origin_transform_error_order where id in (".implode(",", $list).")";
    echo $sql.PHP_EOL;
    $oms_user->query($sql);
}
