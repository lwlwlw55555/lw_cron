...
$sys_seller_nicks = [];
$sys_shop_mapping = [];
$sql_get_sys_jst_shops = "SELECT s.access_token,s.platform_shop_name as platform_user_name from shop s
left join shop_extension se on s.shop_id = se.shop_id
WHERE s.platform_code  = 'TAOBAO' and se.enabled = 1";
$sys_shops = $oms_sync_db->getAll($sql_get_sys_jst_shops);
		...
		if (!empty($resp_add)) {
			...
			if(isset($resp_add['error_response']) || !isset($resp_add['jushita_jdp_user_add_response'])){
				if (strpos(json_encode($resp_add),"invalid-sessionkey") !== false) {
					$shop_id = $oms_sync_db->getOne("select shop_id from shop where access_token = '{$sys_shop_mapping[$value]}'");
					if (!empty($shop_id)) {
						$oms_sync_db->query("update shop_extension set enabled = 0 where shop_id = {$shop_id}");
					}
				}
