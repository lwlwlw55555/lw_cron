...
$from_conf = $jst_db_conf;
global $from_conf;
$jst_db = ClsPdo::getInstance($jst_db_conf);

moveDataByPartyId($jst_db,$lw_db);

function moveDataByPartyId($jst_db,$lw_db){
    
    $sql = "select distinct TABLE_NAME from information_schema.COLUMNS  where TABLE_SCHEMA = 'mddoms_0'";
    $tables = $jst_db->getCol($sql);
    var_dump($tables);
    ...
    foreach ($tables as $table){
        // if (!$is_ok && $table != 'delivery_job') {
            // continue;
        // }
        // if ($table == 'jdp_tb_item') {
            // continue;            
        // }
        $is_ok = true;
        moveData($jst_db,$lw_db,$table);
    }

    // moveOrderGoodsExtension($party_id, 'order_goods_extension', $jst_db, $jst_2_db, $jst_db_conf, $jst_2_db_conf, 'mddoms_0');
    // echo "[insert into db2, party_id: {$party_id}]" .date("Y-m-d H:i:s"). " mddoms_0 party_id end" .PHP_EOL;
...
function moveData($from_db,$to_db,$table){
    ...
    $primay_key = null;
    if(empty($columns)){
        $columns_infos = execRetry($from_db, $from_conf, "show COLUMNS from mddoms_0.{$table}", 'getAll');
