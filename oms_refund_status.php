...
while($is_begin || $count == 2000){
    $is_begin = false;
    $sql = "select order_goods_id from
         order_goods
      limit {$start},2000";
    ...
    $count = count($list);
    $start += $count; 
    $sql = "update order_goods og inner join origin_order_goods oog
            on og.origin_order_goods_id = oog.origin_order_goods_id
        set og.refund_status = oog.refund_status
        where og.order_goods_id in (".implode(",", $list).")";
    echo $sql.PHP_EOL;
    $oms_db->query($sql);
}
